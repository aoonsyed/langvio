...
def is_video(filename):
    return '.' in filename and \
        filename.rsplit('.', 1)[1].lower() in {'mp4', 'mov', 'avi', 'webp'}
...
def process_media():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            if pipeline is None:
                flash('Error: Could not initialize the Langvio pipeline. Please check your installation.')
            ...
            destination = os.path.join(app.root_path, 'static', 'results', output_filename)
            ...
            if is_video_file and 'detections' in result:
                frame_count = len([k for k in result['detections'].keys() if k.isdigit()])
        ...
        flash('File type not allowed. Please upload an image (png, jpg, jpeg) or video (mp4, mov, avi, webm).')
        return redirect(url_for('index'))
...
if __name__ == '__main__':
    ...
    if pipeline is None:
        logger.error("Failed to initialize Langvio pipeline. Check if Langvio is installed correctly.")
    else:
