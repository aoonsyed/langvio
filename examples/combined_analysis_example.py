...
def main():
    ...
    if os.path.exists(image_path):
        ...
        # Analyze colors
        color_summary = defaultdict(lambda: defaultdict(int))
        for det in detections:
            if "attributes" in det and "color" in det["attributes"]:
                color = det["attributes"]["color"]
                color_summary[det["label"]][color] += 1

        print("\nColor analysis:")
        for obj, colors in color_summary.items():
            print(f"- {obj} colors: {', '.join([f'{c}:{n}' for c, n in colors.items()])}")
        ...
        for det in detections:
            for rel in det.get("relationships", []):
                rel_key = f"{det['label']} {rel['relations'][0]} {rel['object']}"
                relationship_summary[rel_key] += 1

        # Show the top 5 most common relationships
        top_relationships = sorted(relationship_summary.items(), key=lambda x: x[1], reverse=True)[:5]
        for rel, count in top_relationships:
    ...
    if os.path.exists(video_path):
        ...
        # Complex analysis combining multiple capabilities
        query = "Analyze this traffic video. Track vehicles, identify their movement patterns, count how many pedestrians cross the street, and note any unusual activities or events."
        print(f"Query: {query}")
        ...
        for frame_key, detections in result["detections"].items():
            ...
            for det in detections:
                ...
                if "track_id" in det:
                    ...
                    tracked_objects[track_id]["positions"].append(det["center"] if "center" in det else None)
        ...
        avg_counts = {label: count / total_frames for label, count in frame_counts_by_type.items()}
