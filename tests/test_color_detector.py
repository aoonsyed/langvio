...
def solid_red_image():
    ...
    return np.ones((100, 100, 3), dtype=np.uint8) * np.array([0, 0, 255], dtype=np.uint8)  # BGR


@pytest.fixture
def solid_green_image():
    """Create a solid green image."""
    return np.ones((100, 100, 3), dtype=np.uint8) * np.array([0, 255, 0], dtype=np.uint8)  # BGR


@pytest.fixture
def solid_blue_image():
    """Create a solid blue image."""
    return np.ones((100, 100, 3), dtype=np.uint8) * np.array([255, 0, 0], dtype=np.uint8)  # BGR
...
class TestColorDetector:
    ...
    def test_detect_color_solid_basic_colors(self, solid_red_image, solid_green_image, solid_blue_image):
    ...
    def test_detect_color_solid_grayscale(self, solid_black_image, solid_white_image, solid_gray_image):
    ...
    def test_detect_color_solid_secondary_colors(self, solid_yellow_image, solid_cyan_image, solid_magenta_image,
                                                 solid_orange_image):
        ...
        assert any(
            c in color.lower() for c in ["cyan", "teal", "turquoise", "blue"]), f"Expected cyan-like color in '{color}'"

        # Test magenta image
        color = ColorDetector.detect_color(solid_magenta_image)
        assert any(c in color.lower() for c in
                   ["magenta", "purple", "violet", "pink"]), f"Expected magenta-like color in '{color}'"

        # Test orange image
        color = ColorDetector.detect_color(solid_orange_image)
        assert "orange" in color.lower() or "red" in color.lower(), f"Expected 'orange' or 'red' in '{color}'"
    ...
    def test_detect_color_with_return_all(self, solid_red_image, split_red_blue_image):
        ...
        assert "red" in dominant_color.lower(), f"Expected 'red' in dominant color '{dominant_color}'"

        # Test split image
        color_percentages = ColorDetector.detect_color(split_red_blue_image, return_all=True)
        ...
        assert has_red and has_blue, f"Expected both red and blue colors, got: {list(color_percentages.keys())}"

    def test_detect_color_threshold(self, split_red_blue_image, split_red_green_blue_image):
        ...
        assert color.lower() == "multicolored" or color.lower() == "unknown", f"Expected 'multicolored' or 'unknown', got '{color}'"
        ...
        assert color.lower() != "multicolored" and color.lower() != "unknown", f"Expected a color name, got '{color}'"
    ...
    def test_detect_colors_layered(self, solid_red_image, split_red_blue_image, rainbow_image):
        ...
        assert "red" in colors[0].lower(), f"Expected 'red' in first color '{colors[0]}'"
        ...
        colors_limited = ColorDetector.detect_colors_layered(rainbow_image, max_colors=2)
        assert len(colors_limited) <= 2, f"Expected at most 2 colors, got: {len(colors_limited)}"

    def test_get_color_profile(self, solid_red_image, split_red_blue_image, noisy_red_image):
    ...
    def test_get_color_name(self):
        ...
        assert any(c in cyan_color.lower() for c in ["cyan", "teal", "turquoise", "blue"])

        magenta_color = ColorDetector.get_color_name((255, 0, 255))
        assert any(c in magenta_color.lower() for c in ["magenta", "purple", "violet", "pink"])
    ...
    def test_visualize_colors(self, solid_red_image, split_red_blue_image, rainbow_image):
        ...
        assert vis_image.shape[0] > 0 and vis_image.shape[1] > 0, "Visualization should have positive dimensions"
        ...
        assert vis_image.shape[0] > 100, "Rainbow visualization should be taller to show many colors"
    ...
    def test_edge_cases(self, invalid_image, single_channel_image, solid_black_image):
        ...
        assert color == "unknown", f"Expected 'unknown' for invalid image, got '{color}'"
        ...
        try:
            ...
            assert color in ["unknown", "gray", "black", "white"], f"Expected grayscale color, got '{color}'"
        except cv2.error:
        ...
        assert vis_image.shape[2] == 3, "Even for invalid input, should return a valid visualization"

        # Test find_objects_by_color with invalid image
        mask = ColorDetector.find_objects_by_color(invalid_image, "red")
        assert isinstance(mask, np.ndarray), "Should return a numpy array even for invalid input"
    ...
    def test_color_ranges(self):
        ...
        for color_range in ColorDetector.COLOR_RANGES:
            assert len(color_range) == 3, "Each color range should have 3 elements (lower, upper, name)"
