...
class ColorDetector:
    ...
    @classmethod
    def detect_color(cls, image_region: np.ndarray, return_all: bool = False, threshold: float = 0.15) -> Union[
        str, Dict[str, float]]:
        """
        ...
        if image_region is None or image_region.size == 0 or image_region.shape[0] == 0 or image_region.shape[1] == 0:
        ...
        color_counts = {}
        nonzero_pixels = np.count_nonzero(hsv_region[:, :, 0] >= 0)  # Total valid pixels
        ...
        for (lower, upper, color_name) in cls.COLOR_RANGES:
            ...
            if percentage > 0.05:  # Only count colors covering at least 5% of the region
    ...
    @classmethod
    def detect_colors_layered(cls, image_region: np.ndarray, max_colors: int = 3) -> List[str]:
        """
    ...
    def visualize_colors(cls, image_region: np.ndarray) -> np.ndarray:
        ...
        # Add title
        cv2.putText(vis_image, "Color Analysis", (10, 30),
        ...
        for i, (color, percentage) in enumerate(profile["color_percentages"].items()):
            ...
            cv2.rectangle(vis_image, (150, y_offset - 15), (150 + bar_width, y_offset),
                          bgr_color.tolist(), -1)
